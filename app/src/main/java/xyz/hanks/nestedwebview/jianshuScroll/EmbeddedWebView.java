...
//        addJavascriptInterface(new JSGetContentHeight(), "InjectedObject");
    ...
    protected void onScrollChanged(int x, int y, int oldx, int oldy) {
        ...
        if (y <= 0) {
            this.position = ScrollStateChangedListener.ScrollState.a;
            return;
        }
...
//            Log.i(TAG,"yy=="+y+"  webviewHeight=="+this.webviewHeight+"  contentHeight=="+this.contentHeight);
            if (y + this.webviewHeight >= this.contentHeight) {
                if (this.contentHeight > 0) {
                    this.position = ScrollStateChangedListener.ScrollState.b;
                }
            } else {
                this.position = ScrollStateChangedListener.ScrollState.c;
            }
        }
...
//                }
                ...
                if (deltaY != 0) {
                    this.direction = this.directionDetector.a(deltaY, true, this.scrollStateChangedListener);
                }
...
//                    setJavaScriptEnable(true);
                    ...
                    switch (this.direction) {
                        case 1: {
                            if ((this.position != ScrollStateChangedListener.ScrollState.b) && (this.contentHeight != this.webviewHeight)) {
                        ...
                        break;
                        case 2:
                            if ((this.position == ScrollStateChangedListener.ScrollState.a) || (this.contentHeight == this.webviewHeight)) {
    ...
    private class JSGetContentHeight {
        ...
        public void getContentHeight(WebView paramWebView, int paramInt) {
            int i = EmbeddedWebView.this.getHeight();
//      if (EmbeddedWebView.this.originHeight == 0)
//        EmbeddedWebView.access$102(EmbeddedWebView.this, i);
            Log.i(EmbeddedWebView.TAG, "JS getContentHeight contentHeight " + paramInt + " originHeight = " + EmbeddedWebView.this.originHeight + " viewHeight = " + i);
            int j = (int) (paramInt * EmbeddedWebView.this.density);
            int k = 0;
            if ((i > 0) && (j > 0)) {
                if (i - j <= 200) {
                    k = 0;
            ...
            if (k > EmbeddedWebView.this.originHeight)
                k = EmbeddedWebView.this.originHeight;
            if ((k > 0) && (k != i)) {
                final int m = k;
                EmbeddedWebView.this.post(new Runnable() {
                    public void run() {
                        ViewGroup.LayoutParams localLayoutParams = EmbeddedWebView.this.getLayoutParams();
                        localLayoutParams.height = m;
                        Log.i(EmbeddedWebView.TAG, "resize height = " + localLayoutParams.height);
                        EmbeddedWebView.this.setLayoutParams(localLayoutParams);
                    }
                });
            }
        }
